...
テンプレートには貪欲・ビーム・局所探索までの土台が入っているが、**最初の提出に全部は不要**。まず問題文を読み、以下だけを順番に行う。途中で次の段階のファイルを開かない。

| 段階 | 自分で編集するもの | 必須か | ここで確認すること |
| --- | --- | --- | --- |
| 1. 入出力 | `src/problem.rs` | 必須 | 入力を読み、提出する出力の型と表示方法を決める |
| 2. 最小解 | `src/bin/valid.rs` | 必須 | とても弱くても合法な出力を一つ作る |
| 3. 公式確認 | コードは増やさない | 必須 | `./ahc run 0 --solver valid --debug` が通り、可視化を説明できる |
| 4. 本命の解 | 下の「道A」か「道B」の**片方** | 必須 | 改善した解を一つ作る |
| 5. 提出 | 編集しない `submit/<solver>.rs` | 必須 | `export` が作った単一ファイルをWebへ貼る |

`tools/`、`target/`、`out/`、`results/`、`submit/` は編集しない。`tools/` は公式採点器、残りはコマンドが作る生成物である。

`valid.rs` はAtCoderへの提出そのものには必須ではない。ただし、この練習では「最小の合法解を公式toolsへ渡す」経験を飛ばさないため、必須段階として扱う。

### 道A: まず一つのファイルに書く（仕組みを学ぶ前の最短ルート）

`src/bin/a.rs` を自分の `main` と `solve` に書き換える。`src/problem.rs` の `read_input` と `print_answer` を呼び、`Output` を返す。`constructive_state.rs`、`greedy.rs`、`framework/` は使わない。

この道では `a.rs` が提出対象である。最初はこれで十分で、問題が小さい、または単純な規則で解を作れるなら最後までこの形でよい。

### 道B: 一手ずつ作る貪欲を使う（構築型の問題向け）

「空の解から、操作・頂点・割当などを一つずつ追加して完成させる」問題なら、テンプレートの分割を使う。この道を選んだ時だけ、次を編集する。

| ファイル | 役割 | 編集するか |
| --- | --- | --- |
| `src/constructive_state.rs` | 途中の解、次に選べる候補、1手進める処理、候補比較用の値 | **編集する** |
| `src/bin/greedy.rs` | `State` を貪欲ループへ渡して出力する入口 | 通常は編集しない |
| `src/framework/constructive.rs` | 貪欲ループそのもの | 編集しない |
| `src/bin/a.rs` | 現在は `greedy.rs` を読み込む提出用ラッパー | 通常は編集しない |

この道では `./ahc run 0 --solver a` が `greedy.rs` を実行する。`a.rs` の `include!("greedy.rs")` を消さない限り、提出時も `--solver a` でよい。

### 今は任意のもの

- `validate_output`: 最初は出力個数・値域だけでよい。公式toolsで最小解が通った後に足す。
- `calculate_score`: 探索の比較や差分更新を始める前には必要。最小解だけなら後回しでよい。
- `./ahc add ...`、ビーム、局所探索、焼きなまし: 貪欲または単一ファイルの解が動き、可視化で失敗を観察してから選ぶ。
- `bench`、`save`: 1ケースを説明できるようになってから使う。最初の1ケース確認には不要。
