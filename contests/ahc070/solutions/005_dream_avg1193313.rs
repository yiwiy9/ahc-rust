// AHC-SOLUTION: ahc070 simulated-annealing alternate
...
// 局所探索の共通部分
//
// 問題固有の処理はSaStateに任せ、この範囲では近傍の採用・巻き戻しと
...
impl LocalSearch {
    fn new(initial_state: SaState, config: SearchConfig) -> Self {
        Self {
    ...
    fn solve(mut self, input: &Input) -> SaState {
...
fn main() {
    ...
    let initial_state = SaState::new(&input, output);
    ...
    let sa_state = LocalSearch::new(initial_state, config).solve(&input);

    let state = solve_beam(&input, State::new(sa_state.output.patterns), BEAM_WIDTH);
