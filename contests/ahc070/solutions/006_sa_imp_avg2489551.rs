// AHC-SOLUTION: ahc070 simulated-annealing improved
...
fn solve_greedy(input: &Input) -> Output {
    ...
    let mut remains = vec![vec![true; GRID_LEN as usize]; GRID_LEN as usize];
    let mut visited_set: HashSet<(i64, i64)> = HashSet::new();
    let mut cur = (0, 0);
    for turn in 0..TURN_COUNT {
        remains[input.strange_positions[turn].0 as usize]
            [input.strange_positions[turn].1 as usize] = false;
        ...
        let action = (0..PATTERN_COUNT)
            .min_by_key(|&pattern| {
                ...
                (min_dist, calc_dist_sum(&remains, next))
            })
            .unwrap();
...
fn calc_dist_sum(remains: &[Vec<bool>], s: (i64, i64)) -> i64 {
    let mut sum = 0;
    for (i, row) in remains.iter().enumerate() {
        for (j, &yes) in row.iter().enumerate() {
            if yes {
                sum += calc_dist(s, (i as i64, j as i64));
            }
        }
    }
    sum
